...
function save(){
  // v60 stability: save data immediately, but avoid full redraw/list rebuilds on every input event.
  try{
...
function render(){
  // v60 stability: do not rebuild the full ChatGPT handover prompt on every field change.
...
document.addEventListener('DOMContentLoaded',()=>{bindCriticalButtons();migrateOldStorageKeys();if($('appVersionBadge'))$('appVersionBadge').innerText='App version: v60';load();initSignaturePad();
...
/* v60 startup rescue and cache tools */
(function(){
  const VERSION='v60';
  const $=id=>document.getElementById(id);
  function setVersion(){
  ...
  function clearDraftFromQuery(){
    ...
    if(q.get('clearDraft')==='1' || q.get('resetDraft')==='1'){
      ...
      const clean=location.origin + location.pathname.replace(/\/?index\.html$/,'/index.html') + '?v60=' + Date.now();
  ...
  function patchUpdateButton(){
    async function cleanCachesOnly(){
      ...
      location.replace(location.origin + location.pathname.replace(/\/?index\.html$/,'/') + '?v60=' + Date.now());
    }
