...
function save(){
  // v62 stability: save data immediately, but avoid full redraw/list rebuilds on every input event.
  try{
...
function render(){
  // v62 stability: do not rebuild the full ChatGPT handover prompt on every field change.
...
document.addEventListener('DOMContentLoaded',()=>{bindCriticalButtons();migrateOldStorageKeys();if($('appVersionBadge'))$('appVersionBadge').innerText='App version: v62';load();initSignaturePad();
...
(function(){
  ...
  function renderQuoteCheck61(q){
    ...
    try{ localStorage.setItem('tlgec_last_priced_quote_v62', JSON.stringify(window.__lgSurveyLastQuoteV62)); }catch(e){}
    return !errors.length;
  }
  ...
  function patchRefreshPresent61(){
    try{
      if(typeof refreshPresent === 'function' && !refreshPresent.v62PricingWrapped){
        const old=refreshPresent;
        const wrapped=function(){
          lockPanelCountToRoofAllocation61();
          return old.apply(this, arguments);
        };
        wrapped.v62PricingWrapped=true;
  ...
  function patchAutoFit61(){
    try{
      if(typeof autoFitRoofPanels === 'function' && !autoFitRoofPanels.v62PricingWrapped){
        const old=autoFitRoofPanels;
        const wrapped=function(force=false){
          const pc=$('panelCount');
          const locked=pc && pc.dataset.v62Priced==='yes' && !force;
          const before=pc ? pc.value : '';
          const out=old.apply(this, arguments);
          if(locked && pc){
            pc.value=before;
  ...
  function bindCalculateButton61(){
    ...
    if(btn && !btn.dataset.v62PricingBound){
      btn.dataset.v62PricingBound='yes';
      btn.addEventListener('click', function(e){
        e.preventDefault();
        e.stopPropagation();
        if(e.stopImmediatePropagation) e.stopImmediatePropagation();
    ...
    if(build && !build.dataset.v62PricingBound){
      build.dataset.v62PricingBound='yes';
  ...
  function bindPanelAllocation61(){
    document.querySelectorAll('#roofPlanes .roofPanels').forEach(el=>{
      if(el.dataset.v62PricingBound) return;
      el.dataset.v62PricingBound='yes';
