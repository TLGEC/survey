...
function scheduleLightRenderV60(){
  if(window.__lgSurveyRenderTimerV60) clearTimeout(window.__lgSurveyRenderTimerV60);
  window.__lgSurveyRenderTimerV60 = setTimeout(() => {
    if(window.__lgSurveyRenderLock) return;
    window.__lgSurveyRenderLock = true;
    try{ render(); }catch(e){}
    finally{ window.__lgSurveyRenderLock = false; }
  }, 120);
}
function save(){
  // v61 stability: save data immediately, but avoid full redraw/list rebuilds on every input event.
  try{
    localStorage.setItem(KEY,JSON.stringify(getData()));
  }catch(e){
    console.warn('LG Survey save failed', e);
    return;
  }
  try{ if(typeof updateHeader === 'function') updateHeader(); }catch(e){}
  const activeId = document.activeElement && document.activeElement.id;
  const lightOnly = ['tigoQty','batteryBrand','sig6Qty','sig10Qty','sigGateway','sigControllerMode','sigBatteryOnlyController','sigInstallType','sig6Price','sig10Price','sigGatewayPrice','teslaSaleType','pw3Qty','dcExpQty','gateway','pw3Price','gatewayPrice','dcPrice','teslaDiscounts','panelCount','panelModel'].includes(activeId);
  if(lightOnly){
    scheduleLightRenderV60();
    return;
  }
  scheduleLightRenderV60();
  // Saved survey lists are only rebuilt while those panels are visible.
  if(document.querySelector('#internal.panel.on')){
    try{ renderSavedList(); }catch(e){}
  }
  if(document.querySelector('#home.panel.on')){
...
function render(){
  // v61 stability: do not rebuild the full ChatGPT handover prompt on every field change.
  // The prompt can be large and quote-heavy, so only refresh it when the Tools panel is actually open.
  try{ updateHeader(); }catch(e){}
  const out=$('out');
  const toolsOpen=!!document.querySelector('#internal.panel.on');
  if(out && toolsOpen){
    try{ out.textContent=prompt(); }catch(e){ out.textContent='Prompt preview could not refresh. Use Download notes TXT or copy again after saving.'; }
  }
}
...
document.addEventListener('DOMContentLoaded',()=>{bindCriticalButtons();migrateOldStorageKeys();if($('appVersionBadge'))$('appVersionBadge').innerText='App version: v61';load();initSignaturePad();
...
/* v61 startup rescue and cache tools */
(function(){
  const VERSION='v61';
  const $=id=>document.getElementById(id);
  function setVersion(){
    const badge=$('appVersionBadge');
    if(badge) badge.innerText='App version: '+VERSION;
  }
  function query(){
    try{return new URLSearchParams(location.search);}catch(e){return new URLSearchParams('');}
  }
  function clearDraftFromQuery(){
    const q=query();
    if(q.get('clearDraft')==='1' || q.get('resetDraft')==='1'){
      try{ localStorage.removeItem('tlgec_current_draft_v1'); }catch(e){}
      const clean=location.origin + location.pathname.replace(/\/?index\.html$/,'/index.html') + '?v61=' + Date.now();
      location.replace(clean);
    }
  }
  function patchUpdateButton(){
    async function cleanCachesOnly(){
      try{
        if('serviceWorker' in navigator){
          const regs=await navigator.serviceWorker.getRegistrations();
          for(const r of regs){try{await r.unregister();}catch(e){}}
      ...
      location.replace(location.origin + location.pathname.replace(/\/?index\.html$/,'/') + '?v61=' + Date.now());
    }
    const a=$('updateApp'), b=$('homeUpdateApp');
    if(a) a.onclick=cleanCachesOnly;
    if(b) b.onclick=cleanCachesOnly;
  }
  function bind(){
    clearDraftFromQuery();
    setVersion();
...
/* v61 pricing consistency patch: Proposal now uses the same priced inputs as the Design quote. */
(function(){
  const VERSION='v61';
  const $=id=>document.getElementById(id);
  const n=v=>Number(v||0)||0;
  ...
  function currentPanelName(){
    try{
      const p=typeof panelParts==='function' ? panelParts() : null;
      return p && p.name ? p.name : (($('panelModel')?.value||'panel').split('|')[0] || 'panel');
    }catch(e){
      return (($('panelModel')?.value||'panel').split('|')[0] || 'panel');
    }
  }

  function roofPanelTotal61(){
    let total=0;
  ...
  function lockPanelCountToRoofAllocation61(){
    ...
    if(roofTotal>0){
      ...
      pc.dataset.v61Priced='yes';
      pc.dataset.pricedPanelCount=String(roofTotal);
      return roofTotal;
    }
    const current=n(pc.value);
    if(current>0){
      pc.dataset.manual='yes';
      pc.dataset.v61Priced='yes';
      pc.dataset.pricedPanelCount=String(current);
    }
    return current;
  }

  function quoteNow61(){
    lockPanelCountToRoofAllocation61();
    try{ if(typeof syncTeslaOptions === 'function') syncTeslaOptions(); }catch(e){}
    try{
      if(typeof quote === 'function') return quote();
    }catch(e){}
    return {total:0,kWp:0,panel:{name:currentPanelName()},batteryText:'',extrasText:''};
  }

  function batteryLabel61(){
    const brand=$('batteryBrand')?.value || 'None';
    if(brand==='Tesla'){
      try{ return brand + ' • ' + teslaConfigText(); }catch(e){ return brand; }
    }
    if(brand==='Sigenergy'){
      try{
        const q=quoteNow61();
        return q.batteryText || brand;
      }catch(e){ return brand; }
    }
    return brand;
  }

  function renderQuoteCheck61(q){
    const box=$('quoteCheck');
    const p=typeof panelParts==='function' ? panelParts() : {name:currentPanelName()};
    const count=n($('panelCount')?.value);
    const brand=$('batteryBrand')?.value || 'None';
    const errors=[];
    const messages=[];
    if($('solar')?.checked && !count) errors.push('No panel count entered.');
    ...
    if(($('battery')?.checked || brand !== 'None') && brand === 'None') errors.push('Battery selected but no battery brand chosen.');
    if(brand === 'Sigenergy'){
      const sigQty = n($('sig6Qty')?.value) + n($('sig10Qty')?.value) + n($('sigModuleQty')?.value);
      if(!sigQty) errors.push('Sigenergy selected but no battery module quantity entered.');
    }
    if(brand === 'Tesla'){
      try{
        const pw3 = typeof teslaPw3Qty === 'function' ? teslaPw3Qty() : n($('pw3Qty')?.value);
        const exp = typeof teslaExpansionQty === 'function' ? teslaExpansionQty() : n($('dcExpQty')?.value);
        if(exp && !pw3) errors.push('DC Expansion requires at least one Powerwall 3.');
        messages.push(typeof teslaConfigText === 'function' ? teslaConfigText() : 'Tesla route selected');
      }catch(e){}
    }

    if(box){
      const roofTotal=roofPanelTotal61();
      const sourceLine=roofTotal>0
        ? `Pricing locked to roof panel allocations: ${roofTotal} panel${roofTotal===1?'':'s'}.`
        : `Pricing locked to manual panel count: ${count} panel${count===1?'':'s'}.`;
      box.innerHTML = `<div class="${errors.length?'quoteWarn':'quoteGood'}">
        <b>${errors.length?'Check before presenting':'Quote calculated'}</b>
        <p>${count ? `${count} x ${p.name}` : 'No panels selected'} ${q.kWp ? ' • ' + q.kWp + ' kWp' : ''}</p>
        <p>Battery: ${batteryLabel61()}</p>
        <p class="quoteTotal">Proposal position: ${money61(q.total || 0)}</p>
        <p>${sourceLine} The Proposal page will use the same priced inputs.</p>
        ${q.extrasText && q.extrasText !== 'No manual extras selected' ? `<div class="quoteExtraLine"><b>Manual extras:</b> ${q.extrasText}</div>` : ''}
        ${messages.length ? '<ul>' + messages.map(x=>'<li>'+x+'</li>').join('') + '</ul>' : ''}
        ${errors.length ? '<ul>' + errors.map(x=>'<li>'+x+'</li>').join('') + '</ul>' : '<p>No obvious missing build items found.</p>'}
      </div>`;
    }
    ...
    const qt=$('quoteTotal');
    if(qt){
      const overrideText=q.override>0?'Approved override used. Included items are listed without cost breakdown.':'Calculated from Residential Pricing V8.6 logic.';
      qt.innerHTML=`<b>Total: ${money61(q.total)}</b><br>${overrideText}<br>${count} x ${p.name}, ${q.kWp} kWp<br>Battery: ${q.batteryText||batteryLabel61()}<br>${q.controllerText?('Controller: '+q.controllerText+'<br>'):''}Bird protection: ${$('bird')?.checked&&$('solar')?.checked?'Included':'Not included'} | Tigo: ${q.tigoQty||0} optimiser(s) | SPDs: ${$('spds')?.checked?'Included':'Not included'} | Scaffold: ${$('scaffoldLifts')?.value||0} lift(s) included<br>Extras: ${q.extrasText||'No manual extras selected'}`;
    }

    window.__lgSurveyLastQuoteV61={
      at:new Date().toISOString(),
      panelCount:count,
      roofPanelTotal:roofPanelTotal61(),
      panelName:p.name,
      kwp:q.kWp,
      total:q.total,
      battery:q.batteryText || batteryLabel61()
    };
    try{ localStorage.setItem('tlgec_last_priced_quote_v61', JSON.stringify(window.__lgSurveyLastQuoteV61)); }catch(e){}
    return !errors.length;
  }

  function runPricing61(options={}){
    const q=quoteNow61();
    const ok=renderQuoteCheck61(q);
    try{ if(typeof renderPanelSenseCheck==='function') renderPanelSenseCheck(); }catch(e){}
  ...
  function patchRefreshPresent61(){
    try{
      if(typeof refreshPresent === 'function' && !refreshPresent.v61PricingWrapped){
        const old=refreshPresent;
        const wrapped=function(){
          lockPanelCountToRoofAllocation61();
          return old.apply(this, arguments);
        };
        wrapped.v61PricingWrapped=true;
        refreshPresent=wrapped;
        window.refreshPresent=wrapped;
      }
    }catch(e){}
  }

  function patchAutoFit61(){
    try{
      if(typeof autoFitRoofPanels === 'function' && !autoFitRoofPanels.v61PricingWrapped){
        const old=autoFitRoofPanels;
        const wrapped=function(force=false){
          const pc=$('panelCount');
          const locked=pc && pc.dataset.v61Priced==='yes' && !force;
  ...
  function bindCalculateButton61(){
    const btn=$('calculateQuote');
    if(btn && !btn.dataset.v61PricingBound){
      btn.dataset.v61PricingBound='yes';
      btn.addEventListener('click', function(e){
        e.preventDefault();
        e.stopPropagation();
        if(e.stopImmediatePropagation) e.stopImmediatePropagation();
    ...
    const build=$('buildPresent');
    if(build && !build.dataset.v61PricingBound){
      build.dataset.v61PricingBound='yes';
  ...
  function bindPanelAllocation61(){
    document.querySelectorAll('#roofPlanes .roofPanels').forEach(el=>{
      if(el.dataset.v61PricingBound) return;
      el.dataset.v61PricingBound='yes';
      el.addEventListener('input', function(){
        const pc=$('panelCount');
        if(pc){
          pc.dataset.v61Priced='';
          pc.dataset.manual='yes';
        }
      }, true);
    });
  ...
  function bindProposalSync61(){
    // Older navigation code owns tab switching. This wrapper ensures the proposal render itself prices from the same panel allocation.
    document.addEventListener('click', function(e){
      ...
      if(tab==='present' || tab==='agreement'){
        lockPanelCountToRoofAllocation61();
        setTimeout(()=>{ try{ runPricing61({skipProposal:false}); }catch(err){} }, 80);
      }
    }, true);
  }
  ...
  function bind(){
    setVersion();
    patchRefreshPresent61();
    patchAutoFit61();
    bindCalculateButton61();
    bindPanelAllocation61();
    bindProposalSync61();

    setTimeout(()=>{ setVersion(); patchRefreshPresent61(); patchAutoFit61(); bindCalculateButton61(); bindPanelAllocation61(); }, 400);
    setTimeout(()=>{ setVersion(); patchRefreshPresent61(); patchAutoFit61(); bindCalculateButton61(); bindPanelAllocation61(); }, 1200);
  }
